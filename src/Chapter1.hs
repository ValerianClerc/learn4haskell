...
closestToZero x y = let closest = min (abs x) (abs y) in
  if closest == abs x
  then x
  else y
...
mid x y z
  ...
  | between y x z = y
  | between z x y = z
  | otherwise = mostCommon x y z
  where
    between a b c = (a > b && a < c) || (a < b && a > c)
    mostCommon a b c = if a == b then a else c
...
sumLast2 n = onesDigit + tensDigit
  where
    onesDigit = abs n `mod` 10
    tensDigit = flip div 10 $ flip mod 100 $ abs n
